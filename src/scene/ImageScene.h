...
class ImageScene : public Scene {
    ...
    std::optional<Intersection> findIntersection (Ray ray,
    ...
    bool ignoreLight  = false) {
        ...
        for (const auto& primitive : primitives) {
            std::optional<Intersection> currentIntersection =
            primitive->intersectWith (ray);
            ...
            ray.start_position + ray.direction * currentIntersection->dist;
            ...
            case Material::Dielectric: {
                ...
                -glm::dot (currentIntersection->normal, ray.direction);
                ...
                glm::vec3 reflectedDirection = glm::normalize (ray.direction -
                2.0f * currentIntersection->normal *
                dot (currentIntersection->normal, ray.direction));
                Ray reflectedRay (intersectionPosition + reflectedDirection * shift,
                reflectedDirection);
                reflectedRay.depth = ray.depth;

                std::optional<Intersection> reflectedIntersection =
                findIntersection (reflectedRay, maxDistance);
                ...
                if (!reflectedIntersection.has_value ()) {
                    ...
                    reflectedColor =
                    reflectionCoefficient * reflectedIntersection->color;
                }
                ...
                ((refractiveIndex1 / refractiveIndex2) * cosThetaIncident - cosThetaRefracted) *
                currentIntersection->normal);
                Ray refractedRay (intersectionPosition + refractedDirection * shift,
                refractedDirection);
                refractedRay.depth = ray.depth;

                std::optional<Intersection> refractedIntersection =
                findIntersection (refractedRay, maxDistance);
                ...
                if (!refractedIntersection.has_value ()) {
                    ...
                    refractedColor =
                    (1 - reflectionCoefficient) * refractedIntersection->color;
                    if (!currentIntersection->isInside) {
